...
const FIELD_MAP = { name: "name", group: "groupId", night: "usedAtNight", hard: "hard", skill: "skillRestricted" };
...
export function vRol() {
  ...
  return `${ph("Roles and groups", "Roles run down the rota in this order. Groups decide the print colour.", '<input id="newRole" class="input input-bordered w-44" placeholder="Role name"><button class="btn btn-primary" data-act="addRole">Add role</button><button class="btn btn-outline" data-act="bulk" data-k="roles">Paste many</button>')}
   <div class="card bg-base-100 shadow-sm border border-base-300 overflow-x-auto"><table class="table table-sm"><thead><tr><th>Role</th><th>Group</th><th class="text-center">Used at night</th><th class="text-center">Hard role</th><th class="text-center">Needs a skill</th><th></th></tr></thead><tbody>${rows || '<tr><td colspan="6" class="opacity-70">No roles yet.</td></tr>'}</tbody></table></div>
   <div class="text-sm text-base-content/70">Hard role: not given on back-to-back nights. Needs a skill: filled first, and only ever by people ticked as qualified. Ticking who is qualified is done on the People screen.</div>
