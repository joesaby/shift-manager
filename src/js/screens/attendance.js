...
import { D, block, dayLabels, activePeople, essentialRolesForDay, isPresent, getStatus, setStatus, roleById, personById, markStale } from "../model.js";
...
export function vAtt() {
  ...
  const info = days.map((d, i) => ({ d: d, need: essentialRolesForDay(i).length, pc: ppl.filter((p) => isPresent(p, i)).length }));
  const stats = info.map((x) => `<div class="stat"><div class="stat-value text-2xl leading-tight">${esc(x.d.label)} <span class="badge badge-sm ${x.d.shift === "Day" ? "badge-warning" : "badge-info"}">${x.d.shift}</span></div><div class="stat-desc mt-1 font-semibold ${x.pc < x.need ? "text-error" : "text-success"}">${x.pc} present, ${x.need} roles to fill</div></div>`).join("");
