...
| H24 | Historic roster: browse saved periods by year / month / block using the same **person × day** layout as Roster (Name + Employee / Shoulder + day columns, role-group colours, Unassigned). Read-only; Print in colour from the saved snap. Older log layouts still render when a person-day snap cannot be rebuilt. Entries saved before the person × day snapshot are converted for display and do **not** show an “unfilled” warning (it would be guessed from today's role list); entries in the current format show the unfilled list as saved |
