...
- [ ] Any new Tailwind utility class used is actually in `src/styles/tailwind.css` — `grep -c '\.CLASSNAME' src/styles/tailwind.css`. That file is a fixed vendored subset, never regenerated by `build.mjs`; a missing class silently renders nothing (`table-fixed`, `w-24`/`w-40`, `min-w-28`, `input-xs`, `leading-snug`, `break-words`, `px-1` were all confirmed missing at least once). Missing → add plain CSS to `app.css` instead.  
